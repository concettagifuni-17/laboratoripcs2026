...
std::vector<BoolVector<int>> initialize_S_vectors(
    const undirected_graph<T>& G,
    const undirected_graph<T>& coalbero
) {
...
template<typename T>
undirected_graph<AuxNode<T>> build_auxiliary_graph(
    const undirected_graph<T>& G,
    const BoolVector<int>& S_i
) {
    undirected_graph<AuxNode<T>> grafo_ausiliario;
    ...
    for (const auto& arco : G.all_edges()) {
        ...
        if (arco_attivo) {
            grafo_ausiliario.add_edge(
                undirected_edge<AuxNode<T>>(u_piu, v_meno)
            );

            grafo_ausiliario.add_edge(
                undirected_edge<AuxNode<T>>(u_meno, v_piu)
            );
        } else {
            grafo_ausiliario.add_edge(
                undirected_edge<AuxNode<T>>(u_piu, v_piu)
            );

            grafo_ausiliario.add_edge(
                undirected_edge<AuxNode<T>>(u_meno, v_meno)
            );
        }
    }
...
BoolVector<int> auxiliary_path_to_incidence_vector(
    ...
    for (std::size_t i = 0; i + 1 < percorso_ausiliario.size(); i++) {
        ...
        undirected_edge<T> arco_originale(nodo1, nodo2);
...
// Conversione di un ciclo scritto come lista di nodi in vettore booleano di incidenza.
...
BoolVector<int> cycle_to_incidence_vector(
    ...
    for (std::size_t i = 0; i + 1 < ciclo.size(); i++) {
        undirected_edge<T> arco(ciclo[i], ciclo[i + 1]);
...
BoolVector<int> find_minimal_cycle(
    ...
    for (const auto& nodo : G.all_nodes()) {
        ...
        std::vector<AuxNode<T>> percorso_ausiliario =
            dijkstra(grafo_ausiliario, sorgente, destinazione);
...
std::vector<BoolVector<int>> depina_minimum_cycle_basis(
    ...
    undirected_graph<T> albero = dfs_tree(G, sorgente);

    undirected_graph<T> coalbero = G - albero;
